...
library(ggmosaic)
# library(ggradar)
# library(vipor) # us counties population data
# library(tigris) # us counties shapefiles
library(scales)
library(datasauRus)
theme_set(theme_bw())

if(!file.exists("data/utah-counties.Rdata")){
  utah_counties <- tigris::counties("UT")
  ...
  save(utah_counties, file = "data/utah-counties.Rdata")
} else {
  load("data/utah-counties.Rdata")
}
```
...
```{r}
#| label: HR-diagram-show
#| fig-cap: "The Hertzsprung Russell diagram. Discovered independently by Ejnar Hertzsprung (1873–1967) and Henry Norris Russell (1877–1957). The diagram plots the color index of the star against the brightness (absolute magnitude) of the star. As a result, it is possible to discern that these two variables are related and change together over a star's life cycle: a hypothesis that only came to be because of this chart."
#| fig-alt: "A scatter plot showing the color index of a star on the x-axis and the absolute magnitude (brightness) of the star on the y-axis. Points are colored by spectral class, which varies from blue to white to yellow to red as the color index increases and the star's temperature decreases. Points are primarily located along a downward-sloping line from the top left to the bottom right, which is labeled the 'main sequence'. There is another set of points which diverges from the main sequence and extends out horizontally in the middle of the graph; these are labeled 'giants', and a few outliers that are above the giant cluster are labeled 'supergiants'. Below the main sequence stars, there are outliers which are labeled 'dwarfs'."

knitr::include_graphics("images/HR-diagram-1.png")
```
...
- Statistical Summaries are often not enough
- below: summaries of thirteen datasets of 142 observations each
```{r}
...
- Cartesian Coordinates <i class="fa-solid fa-table-cells-large"></i>
- Slope, Intercept form <i class="fa-solid fa-chart-line"></i>
- Functions, Polynomials <i class="fa-solid fa-bezier-curve"></i>
- Probability    
![dotplot](images/geom-dotplot.png){width="40" fig-alt="geom_dotplot"} ![histogram](images/geom-histogram.png){width="40" fig-alt="geom_histogram"} ![boxplot](images/geom-boxplot.png){width="40" fig-alt="geom_boxplot" style="padding-bottom: 12px"}
...
- Polar Coordinates
- Functions - Trigonometry <i class="fa-solid fa-water"></i>, Logarithms🪵, Exponential growth <i class="fa-solid fa-superscript"></i>
- Connecting equations & inequalities to graphs
...
- Science: data collection📋, inference, estimation of constants 🌳🍎

- Civics: election maps🗺️, census, polling, charts about current events

- Art: [personal data collection/visualization](https://towardsdatascience.com/create-beautiful-art-from-your-personal-data-9dc0abfeeaf/), [generative](https://flametree.djnavarro.net) [art](https://art.djnavarro.net/gallery/)

- Media Literacy: [best/worst practices](https://empirestats.net/2026/03/26/misleading-graphs-examples/), claims & support

::: callout-tip
The US Census commissioned Statistical Atlases for the [1870](https://www.loc.gov/item/05019329/), [1880](https://www.loc.gov/item/a40001834/), and [1890](https://www.loc.gov/item/07019233) censuses. They have *amazing* data visualizations of the US population during westward expansion. 
:::
